\documentclass[12px]{report}

%
%   Packages
%
\usepackage[a4paper,top=2cm,bottom=2cm,left=2cm,right=2cm]{geometry}
\usepackage{graphicx}
\usepackage{caption}
\usepackage{subcaption}
\usepackage[sorting=none]{biblatex}
\usepackage[acronym]{glossaries}
\usepackage[english]{babel}
\usepackage{csquotes}
\usepackage{setspace}
\usepackage{makecell}
\usepackage{float}
\usepackage{minted}

%
%   Line spacing
%
\setstretch{1.5}


%
%   Utilities and costants
%
\def\blankpage{
    \clearpage
    \thispagestyle{empty}
    \addtocounter{page}{-1}
    \null
    \clearpage
}

%
%   Acronyms
%
\input{formats/acronyms.tex}
\makenoidxglossaries

%
%   References
%
\addbibresource{formats/references.bib}

%
%   Document
%
\begin{document}
    \input{includes/titlepage.tex}
    %\blankpage
    \input{includes/abstract.tex}
    \clearpage
    \tableofcontents{}
    \clearpage
    \listoffigures
    \clearpage
    \printnoidxglossary[type=acronym, title={List of abbreviations}, style=index, nonumberlist]
    \printacronyms
    \clearpage
    \input{includes/introduction.tex}
    \clearpage
    \input{includes/ariaoveriew.tex}
    \clearpage
    \input{includes/systemarchitecture.tex}
    \clearpage
    \input{includes/sensors.tex}
    \clearpage
    \input{includes/flight-control.tex}
    \clearpage
    \input{includes/data-acquisition.tex}
    \clearpage
    \input{includes/testflight.tex}
    \clearpage
    \input{includes/conclusions.tex}
    \clearpage
    \appendix
    \input{includes/appendix.tex}
    \clearpage
    \printbibliography[
        heading=bibintoc,
        title={Bibliography}
    ]
\end{document}